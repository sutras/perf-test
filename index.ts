...
async function perfTest(time: number, callback: (index: number) => void) {
  perfTest.time = time
  perfTest.progress(0)
...
perfTest.progress = (endedNum: number) => {
  console.clear()
  console.log(
    `当前进度: [${
      ''.padEnd(endedNum, '=') + ''.padEnd(perfTest.time - endedNum, ' ')
    }]`,
  )
}

perfTest.start = (label: string) => {
  const log = new Log(label)
  log.start()
  logs.push(log)
}

perfTest.end = (label: string) => {
    ...
    if (endedNum === perfTest.time) {
      ...
      console.log(
        `平均时间： ${
          logs
            .map((log) => log.getTime())
            .reduce((sum, time) => sum + time, 0) / logs.length
        }ms`,
      )
    }
  } else {
